...
function getClickedRegions(coords, startfrom) {
	let clickedRegions = startfrom || [];
	if (coords.lng) coords = [coords.lng, coords.lat];
	for (let i = 0; i < regions.length; i++) {
		if (turf.booleanPointInPolygon(coords, regions[i]))
...
Promise.all(fetchPromises).then(_ => {
	...
	points.forEach(geoJSON => {
		console.log(geoJSON);
		let marker = new L.marker(swap(geoJSON.geometry.coordinates), { opacity: 0 });
		marker.bindTooltip("⟨"+geoJSON.properties.description+"⟩", {
			permanent: true,
			className: "my-label",
			offset: [-15, 29],
			direction: "top",
		...
		marker.bindPopup(getClickedRegions(geoJSON.geometry.coordinates, [makePopup(geoJSON.properties)]));
		marker.addTo(map);
	});
});